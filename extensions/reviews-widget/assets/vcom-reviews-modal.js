(function () {
  ...
  var modal = null;
  var meta = {
    name: "", url: "/", category: "", logo: "", appLogo: "",
  ...
  function build() {
    ...
    el.innerHTML =
      ...
      '<span class="vcom-rm__decoicons" aria-hidden="true">' +
      ...
      '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"><path d="M4 7h16M4 12h16M4 17h16"/></svg>' +
      "</span>" +
  ...
  function renderHeader(data) {
    ...
    var infoHtml =
      ...
      "</div>";
    head.insertAdjacentHTML("beforeend", infoHtml);
    if (meta.showDisclosure) {
      var noteTitle = meta.disclosureTitle ||
        ("Reviews here are published by " + meta.name + ", and the score is calculated automatically");
      var noteParas = meta.disclosureBody
        ? meta.disclosureBody.split("\n").filter(Boolean).map(function (t) {
            return "<p>" + esc(t) + "</p>";
          }).join("")
      ...
      head.insertAdjacentHTML(
        ...
        "</svg></span>" +
        '<span class="vcom-rm__notet">' + esc(noteTitle) + "</span>" +
        '<span class="vcom-rm__chev" aria-hidden="true">' +
        '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"/></svg>' +
        "</span></button>" +
        '<div class="vcom-rm__notebody" data-rm-note-body hidden>' + noteParas + "</div></div>",
      );
    }

    // Resumo automatico + assuntos mais citados (calculados sobre as avaliacoes reais)
    var extra = "";
    if (meta.showSummary && data.summary_text) {
      extra +=
    ...
    if (meta.showThemes && data.themes && data.themes.length) {
  ...
  function open(trigger) {
    ...
    meta.brandName = trigger.getAttribute("data-brand-name") || "";
    meta.brandLogo = trigger.getAttribute("data-brand-logo") || meta.appLogo;
    meta.accentColor = trigger.getAttribute("data-accent-color") || "";
    meta.showWrite = trigger.getAttribute("data-show-write") !== "0";
    meta.showSummary = trigger.getAttribute("data-show-summary") !== "0";
    meta.showThemes = trigger.getAttribute("data-show-themes") !== "0";
